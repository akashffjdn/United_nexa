...
export const GcEntryList = () => {
  ...
  // ---------------------------------------------------------------------------
  // Selection state (same pattern as TripSheetList)
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Selection helpers (same logic as TripSheetList)
  // ---------------------------------------------------------------------------
  ...
  const isAllVisibleSelected =
    ...
    paginatedData.every((gc) => isRowSelected(gc.gcNo));

  const isIndeterminate = useMemo(() => {
    if (paginatedData.length === 0) return false;
    const selectedCount = paginatedData.filter((gc) =>
      isRowSelected(gc.gcNo)
    ).length;
    return selectedCount > 0 && selectedCount < paginatedData.length;
  }, [paginatedData, selectAllMode, excludedGcNos, selectedGcNos]);

  // Count of actually selected rows (for print and UI)
  const finalCount = selectAllMode
    ? Math.max(0, totalItems - excludedGcNos.length)
  ...
  const isAllSelected =
    ...
    (totalItems > 0 &&
      !selectAllMode &&
      selectedGcNos.length === totalItems);
  ...
  const handleSelectAllVisible = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (checked) {
      if (selectAllMode) {
        // in bulk mode, selecting visible = remove them from exclusions
        setExcludedGcNos((prev) =>
          prev.filter((id) => !visibleGcNos.includes(id))
        );
      } else {
        // normal mode, selecting visible = add to selected list
        setSelectedGcNos((prev) =>
          Array.from(new Set([...prev, ...visibleGcNos]))
        );
      }
    } else {
      if (selectAllMode) {
        // in bulk mode, deselect visible = add them to exclusions
        setExcludedGcNos((prev) =>
          Array.from(new Set([...prev, ...visibleGcNos]))
        );
      } else {
        // normal mode, deselect visible = remove from selected list
        setSelectedGcNos((prev) =>
          prev.filter((id) => !visibleGcNos.includes(id))
        );
      }
  ...
  const handleExcludeFilteredData = () => {
    ...
    else if (filters.consignee && filters.consignee.length > 0)
      filterKey = "Consignee";
  ...
  const handlePrintSelected = async () => {
    ...
    try {
      ...
      if (selectAllMode) {
        // print all filtered except excluded
        printData = await fetchGcPrintData([], true, {
          ...filters,
          excludeIds: excludedGcNos,
        });
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-2 sm:gap-4 items-center justify-between bg-background p-4 rounded-lg shadow border border-muted">
        ...
        {/* RIGHT: Actions - mobile 2 columns, desktop row */}
        <div className="w-full md:w-auto mt-2 md:mt-0 grid grid-cols-3 gap-2 md:flex md:flex-row md:gap-2 md:justify-stretch">
          ...
          {/* BULK BUTTON (Select All / Clear Selection) */}
          <Button
        ...
        <div className="p-4 bg-muted/20 rounded-lg border border-muted animate-in fade-in slide-in-from-top-2">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-2 items-center">
              {/* Exclude button visible when multiple are selected */}
              {multipleSelected && (
                <button
          ...
          {exclusionFilter.isActive && selectAllMode && (
            <div className="mb-4 p-3 bg-yellow-100 text-yellow-800 border border-yellow-300 rounded text-sm">
              <strong>Exclusion Active:</strong> {excludedGcNos.length} GCs are currently
                    ...
                    >
                      <td className="px-4 py-4">
                        <input
                          ...
                          checked={isSelected}
                          onChange={(e) =>
                            handleSelectRow(gc.gcNo, e.target.checked)
                          }
                          title={
                            isSelected
                              ? "Click to Deselect"
        ...
        <div className="block md:hidden divide-y divide-muted">
          {paginatedData.map((gc) => {
            ...
            return (
              <div
                key={gc.id}
                className={`p-4 hover:bg-muted/30 transition-colors border-b border-muted last:border-0 ${
                  isSelected ? "" : ""
                }`}
              >
      ...
      />
      {printingJobs && (
        <GcPrintManager jobs={printingJobs} onClose={() => setPrintingJobs(null)} />
      )}
    </div>
  );
};
