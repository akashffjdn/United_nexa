          ...
          {contentItems.map((item: any, idx: number) => {
            ...
            return (
              <div key={idx} className="border border-border rounded-xl overflow-hidden bg-card shadow-sm">
                <div className="bg-muted/30 px-4 py-2.5 border-b border-border flex justify-between items-center sticky top-0 z-10">
                  ...
                  <span className="text-xs text-muted-foreground font-mono bg-card px-2 py-0.5 rounded border border-border">
                    Range: {startNo} - {endNo}
                  </span>
                </div>

                <div className="p-4 grid grid-cols-5 sm:grid-cols-8 md:grid-cols-10 lg:grid-cols-12 gap-1.5 max-h-60 overflow-y-auto">
                  {fullRange.map(num => {
                    ...
                    if (isLoaded && !wasLoaded) {
                      className = "bg-emerald-100 text-emerald-700 border-emerald-400 font-bold ring-1 ring-emerald-400/30";
                      title = "Newly Loaded";
                      ...
                      className = "bg-card text-muted-foreground border-gray-200 border-dashed hover:bg-muted/20";
                      title = "Remaining";
                    }
                    ...
                    return (
                      <div
                        key={num}
                        title={title}
                        className={`px-1 py-1 rounded text-[10px] sm:text-xs font-mono border flex items-center justify-center min-w-[28px] sm:min-w-[32px] transition-colors ${className}`}
                      >
                        {num}
                      </div>
                    );
    ...
    <div className="space-y-4">
                  ...
                  {logs.length > 0 ? (
                    logs.map((log) => (
                      <tr key={log._id} className="hover:bg-muted/30 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span className="text-sm text-foreground">{log.changedBy}</span>
                        </td>
                        <td className="px-4 py-3">
                          <span className="text-xs text-muted-foreground font-mono">{formatDateTime(log.timestamp)}</span>
                        </td>
                        <td className="px-4 py-3">
      ...
      {selectedLog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-card rounded-xl shadow-2xl w-full max-w-3xl max-h-[85vh] flex flex-col overflow-hidden border border-border">
            <div className="px-4 md:px-6 py-3 md:py-4 border-b border-border flex items-center justify-between bg-muted/30">
              <div>
                <h3 className="text-base md:text-lg font-bold text-foreground flex items-center gap-2">
                  Audit Details
                  <span className="px-2 py-0.5 text-xs font-mono bg-card border border-border rounded text-muted-foreground">
                        ...
                        <table className="w-full text-sm">
                          <thead className="bg-muted/30 text-xs text-muted-foreground uppercase border-b border-border">
                            <tr>
                              ...
                              <th className="px-3 py-2 md:px-4 md:py-2 text-left text-emerald-600 bg-emerald-50/50 w-1/3">New</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-border">
                            {selectedLog.changes.map((change, idx) => (
                              <tr key={idx} className="hover:bg-muted/30">
                                <td className="px-3 py-2 md:px-4 md:py-3 font-medium text-foreground border-r border-border text-xs md:text-sm">
                                  {change.field}
                                </td>
                                <td className="px-3 py-2 md:px-4 md:py-3 text-gray-600 border-r border-border bg-red-50/10 font-mono text-[10px] md:text-xs break-all">
                                  {formatValue(change.oldValue)}
                                </td>
                                <td className="px-3 py-2 md:px-4 md:py-3 text-gray-900 bg-emerald-50/10 font-mono text-[10px] md:text-xs break-all">
            ...
            <div className="p-3 md:p-4 border-t border-border bg-muted/30 flex justify-end">
              <Button
                variant="outline"
                onClick={() => setSelectedLog(null)}
